...
Identical architecture and protocol to the project's stacking model — one
shrinkage-LDA per family (erp, bp, tf, riem) fused by a logistic meta-learner —
the ONLY change is that every family is computed on the 7 channels.
...
CONFIG = {
    ...
    "stats_path": os.path.join(HERE, "xon_stacking.json"),
}
