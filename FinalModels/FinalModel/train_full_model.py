...
train_conn_model.py
===================
Like FinalModel/train_final_model.py, but the 4th feature family is CONNECTIVITY
(`conn`, phase-based PLV/iPLV between fronto-central / central / parietal regions
— the per-trial form of the ConnectivityAnalysis findings) INSTEAD of Riemannian
covariance. Trained on the 3-CLASS dataset (no_stim / medium / max), all 60
channels, with leave-one-subject-out (LOSO) as the headline validation.

Ensemble: erp + bp + tf + conn, one shrinkage-LDA per family fused by a logistic
meta-learner. This is a direct A/B test of connectivity vs Riemannian as the 4th
family (FinalModel is the riem counterpart on the same 3-class dataset).

What it does
------------
  1. Loads data/og-ds-t-3c and extracts the four families PER SUBJECT, IN PARALLEL
     (joblib, path-based so the raw arrays never hit temp).
  2. Fits the deployable stack on ALL subjects (base LDAs + meta-learner on
     leakage-safe out-of-fold base probabilities).
  3. Estimates generalisation with LOSO — the 29 folds run IN PARALLEL.
  4. Saves the trained model + weights to THIS directory:
        ConnModel/conn_stacking_ensemble.joblib
        ConnModel/model_card.json

Run from the repo root:
    PYTHONPATH=ConnModel python ConnModel/train_conn_model.py [--jobs N]
"""
...
CONFIG = {
    "data_folder": "data/og-ds-t-3c",                        # 3-class, all 60 channels
    "families":    ["erp", "bp", "tf", "riem", "conn"],      # 5-family stack (riem AND conn)
    "feature_opts": {
        "erp":  {"feature_set": "full"},
        "bp":   {"channel_set": "all60"},
        "tf":   {"channel_set": "all60"},
        "riem": {"channel_set": "all60"},
        "conn": {},                                          # default FC/C/P regions, 5 bands, PLV+iPLV
    },
    "inner_splits":     5,
    "meta_C":           1.0,
    "artifact_filter":  True,
    "per_subject_norm": True,
    "model_path":  os.path.join(HERE, "full_stacking_ensemble.joblib"),
